...
);isc.evalBoundary;isc.B.push(isc.A.getCriteriaFields=function isc_DataSource_getCriteriaFields(_1){return isc.DS.getCriteriaFields(_1,this)}
...
_1.data=_3}
var _4=this.getDataProtocol(_1);if(_4=="clientCustom")return;if((!this.clientOnly&&!this.cacheAllData)||(_1.cachingAllData||(this.cacheAllData&&_1.operationType&&_1.operationType!="fetch")))
...
this.addDefaultCriteria(_1,this.getOperationBinding(_1));this.applySendExtraFields(_1);var _5=this.getOperationBinding(_1);if(_5==null)_5={};var _6=_5.defaultParams||this.defaultParams;if(_6){_1.data=isc.addProperties({},_6,_1.data)}
_3=_1.data;return this.performSCServerOperation(_1,_3)}}
var _7=this.getServiceInputs(_1);if(_7.dataProtocol=="clientCustom")return;var _8=isc.addProperties({},_1,_7);_8.$374=_1;if(_7.data==null)_8.data=null;if(this.clientOnly||(this.cacheAllData&&_1.operationType=="fetch")){_8.clientOnly=true;_8.callback={target:this,methodName:"$50e"};isc.RPC.sendRequest(_8);return}
var _9=this.getOperationBinding(_1);_8.transport=_9.dataTransport||this.dataTransport;if(_8.transport=="scriptInclude"){_8.callback={target:this,methodName:"$377"};if(!_8.callbackParam){_8.callbackParam=_9.callbackParam||this.callbackParam}
return}
var _2=this.getDataFormat(_1);if(_2=="xml"){var _10=_8.spoofedResponse;if(!_10){_8.callback={target:this,method:this.$378};isc.xml.getXMLResponse(_8)}else{var _11=this;isc.Timer.setTimeout(function(){_11.$378(isc.xml.parseXML(_10),_10,{status:0,httpResponseCode:200,data:_10},_8)})}}else if(_2=="json"){_8.callback={target:this,method:this.$379};isc.rpc.sendProxied(_8)}else if(_2=="csv"){_8.callback={target:this,method:this.$69k};isc.rpc.sendProxied(_8)}else{_8.serverOutputAsString=true;_8.callback={target:this,method:this.$38a};isc.rpc.sendProxied(_8)}}
...
);isc.evalBoundary;isc.B.push(isc.A.dropOnChange=function isc_DataSource_dropOnChange(_1,_2,_3){if(isc.getKeys(_2).length!=isc.getKeys(_1).length)return-1;for(var _4 in _2){var _5=_2[_4],_6=_1[_4];if(isc.isAn.Array(_5)){if(!isc.isAn.Array(_6))return-1;if(_5.length!=_6.length)return-1;if(_5.intersect(_6).length!=_5.length)
...
return 1}
return _7}
);isc.B._maxIndex=isc.C+161;isc.A=isc.DataSource;isc.B=isc._allFuncs;isc.C=isc.B._maxIndex;isc.D=isc._funcClasses;isc.D[isc.C]=isc.A.Class;isc.A.$628=0;isc.A.$71l={sum:function(_1,_2,_3){var _4=0;for(var i=0;i<_2.length;i++){var _6=_1[_2[i].name],_7=parseFloat(_6);if(isc.isA.Number(_7)&&_7==_6){_4+=_7}else{if(_6!=null&&_6!=isc.emptyString){return null}}}
...
if(this.initialData){this.prepareSparseInitialData();this.fillCacheData(this.initialData);this.setFullLength(this.initialLength||this.totalRows||this.initialData.length);if(this.sortSpecifiers)this.setSort(this.sortSpecifiers,true);if(this.sortBy)this.setSort(isc.DS.getSortSpecifiers(this.sortBy),true)}else if(this.isPaged()){this.localData=[]}
...
var _9=this.getValues();if(this.dataSource.sparseUpdates||this.dataSource.noNullUpdates){for(var _10 in _9){var _11=_9[_10];var _12=_2.oldValues[_10];if(this.dataSource.noNullUpdates&&_11===null){delete _9[_10]}else if(this.dataSource.sparseUpdates){var _13=this.dataSource.getField(_10);if(_13&&_13.primaryKey)continue;if(isc.isA.Date(_11)&&Date.compareDates(_11,_12)==0){delete _9[_10]}else if(_11==_12){delete _9[_10]}}}}
if((isc.DynamicForm&&isc.isA.DynamicForm(this)&&this.isMultipart())||this.canSubmit||_8)
...
);isc.B._maxIndex=isc.C+3;(function(){var Offline={isOffline:function(){var offline=window.navigator.onLine?false:true;return offline},KEY_PREFIX:"isc:",LOCAL_STORAGE:"localStorage",GLOBAL_STORAGE:"globalStorage",DATABASE_STORAGE:"databaseStorage",GEARS_DATABASE_API:"gears database api",USERDATA_PERSISTENCE:"userData persistence",GOOGLE_GEARS:"google gears",NO_MECHANISM:"no discovered mechanism",maxResponsesToPersist:100,localStorageType:function(){if(window.localStorage)return this.LOCAL_STORAGE;if(window.globalStorage)return this.GLOBAL_STORAGE;if(window.openDatabase)return this.DATABASE_STORAGE;return this.NO_MECHANISM},getStoredValuesCount:function(){switch(this.localStorageType()){case this.LOCAL_STORAGE:return localStorage.length;case this.GLOBAL_STORAGE:case this.DATABASE_STORAGE:case this.GEARS_DATABASE_API:case this.USERDATA_PERSISTENCE:case this.GOOGLE_GEARS:this.logError("Persistence method '"+this.localStorageType()+"' not yet supported");break}},clearCache:function(force){var count=this.getStoredValuesCount();while(this.getStoredValuesCount()>0){this.removeOldestEntry();if(this.getStoredValuesCount()==count)break;count=this.getStoredValuesCount()}},logCacheContents:function(maxEntryLength){var contents=this.getCacheContents();this.logWarn("Dumping the contents of the browser's local storage:");for(var key in contents){var value=contents[key];if(value&&value.length>maxEntryLength){value=value.substring(0,maxEntryLength)}
this.logWarn(key+" ===> "+value)}},getCacheContents:function(){var index=0,contents={},count=this.getStoredValuesCount();while(index<count){var key=this.getKeyForIndex(index);if(key.indexOf(this.KEY_PREFIX)==0)key=key.substring(this.KEY_PREFIX.length);contents[key]=this.get(key);index++}
return contents},getCacheKeys:function(){var index=0,keys=[],count=this.getStoredValuesCount();while(index<count){var key=this.getKeyForIndex(index);keys[keys.length]=key;index++}
return keys},getKeyForIndex:function(index){switch(this.localStorageType()){case this.LOCAL_STORAGE:return localStorage.key(index);break;case this.GLOBAL_STORAGE:case this.DATABASE_STORAGE:case this.GEARS_DATABASE_API:case this.USERDATA_PERSISTENCE:case this.GOOGLE_GEARS:this.logError("Persistence method '"+this.localStorageType()+"' not yet supported");break}},removeOldestEntry:function(force){switch(this.localStorageType()){case this.LOCAL_STORAGE:var index=0;while(true){var key=localStorage.key(index);if(key&&key.substring(0,this.KEY_PREFIX.length)==this.KEY_PREFIX||force){localStorage.removeItem(key);break}else if(!key){break}}
break;case this.GLOBAL_STORAGE:case this.DATABASE_STORAGE:case this.GEARS_DATABASE_API:case this.USERDATA_PERSISTENCE:case this.GOOGLE_GEARS:this.logError("Persistence method '"+this.localStorageType()+"' not yet supported");break}},put:function(key,value){var ts=new Date().getTime();switch(this.localStorageType()){case this.LOCAL_STORAGE:localStorage.setItem(this.KEY_PREFIX+key,value);break;case this.GLOBAL_STORAGE:case this.DATABASE_STORAGE:case this.GEARS_DATABASE_API:case this.USERDATA_PERSISTENCE:case this.GOOGLE_GEARS:this.logError("Persistence method '"+this.localStorageType()+"' not yet supported");break}
var end=new Date().getTime();this.logWarn("put() with key: "+key+"\nitem: "+this.echoLeaf(value)+": "+(end-ts)+"ms")},get:function(key){var ts=new Date().getTime(),item;switch(this.localStorageType()){case this.LOCAL_STORAGE:var item=localStorage.getItem(this.KEY_PREFIX+key);break;case this.GLOBAL_STORAGE:case this.DATABASE_STORAGE:case this.GEARS_DATABASE_API:case this.USERDATA_PERSISTENCE:case this.GOOGLE_GEARS:this.logError("Persistence method '"+this.localStorageType()+"' not yet supported");break}
var end=new Date().getTime();this.logWarn("get() with key: "+key+"\nitem is: "+this.echoLeaf(item)+": "+(end-ts)+"ms");return item},remove:function(key){this.logWarn("Removing item for key: "+key);switch(this.localStorageType()){case this.LOCAL_STORAGE:localStorage.removeItem(this.KEY_PREFIX+key);break;case this.GLOBAL_STORAGE:case this.DATABASE_STORAGE:case this.GEARS_DATABASE_API:case this.USERDATA_PERSISTENCE:case this.GOOGLE_GEARS:this.logError("Persistence method '"+this.localStorageType()+"' not yet supported");break}},storeResponse:function(dsRequest,dsResponse){var ts=new Date().getTime();var trimmedRequest=this.trimRequest(dsRequest),key=this.serialize(trimmedRequest),value=this.serialize(this.trimResponse(dsResponse));this.logWarn("storeResponse serializing: "+(new Date().getTime()-ts)+"ms");if(this.get(key)==null){if(this.getStoredValuesCount()>=this.maxResponsesToPersist){this.removeOldestEntry()}}
this.put(key,value)},trimRequest:function(dsRequest){var keyProps=["dataSource","operationType","operationId","criteria","values","sortBy","startRow","endRow"],trimmed={},undef;for(var i=0;i<keyProps.length;i++){if(dsRequest[keyProps[i]]!==undef){trimmed[keyProps[i]]=dsRequest[keyProps[i]]}}
return trimmed},trimResponse:function(dsResponse){var keyProps=["dataSource","startRow","endRow","totalRows","data","status","errors","invalidateCache","cacheTimestamp"],trimmed={},undef;for(var i=0;i<keyProps.length;i++){if(dsResponse[keyProps[i]]!==undef){trimmed[keyProps[i]]=dsResponse[keyProps[i]]}}
return trimmed},getResponse:function(dsRequest){var trimmedRequest=this.trimRequest(dsRequest),key=this.serialize(trimmedRequest),value=this.get(key),returnValue;eval('returnValue = '+value);return returnValue},serialize:function(obj){return isc.Comm.serialize(obj,false)}};if(window.isc){isc.defineClass("Offline").addClassProperties(Offline)}else{isc.addProperties=function(objOne,objTwo){for(var propName in objTwo)objOne[propName]=objTwo[propName]}
isc.addProperties(isc.Offline,{serialize:function(object){return isc.OfflineJSONEncoder.encode(object)},logWarn:function(message){if(console)console.log(message)},logError:function(message){if(console)console.log(message);alert(message)},echoLeaf:function(obj){var output="",undefined;if(obj===undefined)return"undef";try{if(typeof obj=="Array"){output+="Array["+obj.length+"]"}else if(typeof obj=="Date"){output+="Date("+obj.toShortDate()+")"}else if(typeof obj=="Function"){output+=isc.Func.getName(obj,true)+"()"}else{switch(typeof obj){case"string":if(obj.length<=40){output+='"'+obj+'"';break}
