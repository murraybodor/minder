    ...
    for (int i = 0; i < tableNames.length; i++) {
        ...
        try {
            dsConfig = DataSourceTools.getDataSourceConfigFromTable(
                            tableNames[i], schemaName, dbName, "sql", timestampType, 
        ...
        if ("js".equals(outputFormat)) {
            ...
            DataSource work = DataSource.fromConfig(dsConfig);
        ...
        if ("true".equals(dumpTestData)) {
            ...
            if ("js".equals(outputFormat)) {
                ...
                xmlOut.write("<List>\n");
                for (Iterator data = dataList.iterator(); data.hasNext(); ) {
                    XML.recordToXML(dsID, (Map) data.next(), xmlOut, true);
                }
                xmlOut.write("</List>");
                xmlString = "<!-- Auto-generated from database table " + tableNames[i] +
                            " -->\n\n" + xmlOut.toString();
...
if (classNames != null) {
    for (int i = 0; i < classNames.length; i++) {
        ...
        if ("js".equals(outputFormat)) {
            ...
            JSTranslater.get().toJS(work, jsOut);
            String jsString = jsOut.toString();
            String comment = "//  Auto-generated from Hibernate mapping " + classNames[i] + "\n\n";
            if (!fromMapping) {
                comment = "// Auto-generated from Java class " + classNames[i] + " -->\n\n";
            }
...
<%! 
	String toXML(Map dsConfig) throws Exception {
        ...
        for (Iterator fields = fieldList.iterator(); fields.hasNext(); ) {
        	xmlOut.write("\t\t");
            XML.recordToXML("field", (Map) fields.next(), xmlOut, true);
        }
        xmlOut.write("\t</fields>\n");
        xmlOut.write("</DataSource>");
